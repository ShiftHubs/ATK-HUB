...
local HWID = {
   "6BFBC34A-A4A8-492B-8212-5CEE96B36300",
   "HWID",
   "HWID",
   "HWID",
   "HWID",
...
function RadientPaid:Window(text,maincolor)
   ...
   UserName.Text = "ATK HUB"
   UserTag.Text = "#" .. " ATK HUB"
   ...
   return ServerHold
	...
	local serv = win:Server("ATK HUB", "")
   local Main = serv:Channel("Auto Farm")

     _G.FarmLevel = false
     Main:Toggle("Auto Farm",_G.FarmLevel,function (vu)
   _G.FarmLevel = vu
    end)

   _G.FastAttcak = true
	Main:Toggle("Fast Attack",_G.FastAttcak,function(vu)
	    _G.FastAttcak = vu
	end)
   ...
   spawn(function()
        game:GetService("RunService").RenderStepped:Connect(function()
            pcall(function()
                if _G.FastAttcak then
            local Combat = require(game:GetService("Players").LocalPlayer.PlayerScript.CombatFarmework)
            local Cemara = require(game:GetService("Players").LocalPLayer.PlayerScript.CombatFarmework.CameraShaker)
            Cemara.CameraShakeInstance.CameraShakeState = {
    FadingIn = 3,
    FadingOut = 2,
    Sustained = 0,
    Inactive = 1
}
                Combat.aciveController.timeToNextAttack = 0.01
                Combat.aciveController.hitboxMagnitude = 120
                Combat.aciveController.incrememt = 3
            end
        end)
    end)
end)
        ...
        game:GetService("RunService").RenderStepped:Connect(function()
            pcall(function()
                if _G.FastAttcak then
                    ...
                    game:GetService'VirtualUser':Button1Down(Vector2.new(3000, 1000))
            end
        end)
    end)
end)
...
function CheckLevel()
    ...
    if MyLevel == 1 or MyLevel <= 1 then
        Ms = "Bandit [Lv. 5]"
        NameMon = "Bandit"
